...
export class RelationsComponent implements OnInit {
  constructor(private fb: FormBuilder, private infoGetterService: InfoGetterService, private _loadingService: LoadingService, private _langService: LangService, private router: Router) {}

  findRelationForm: FormGroup;
  persons;
  treepath;
  showTreeFlag: Boolean = false;
  noPathFlag: Boolean = false;
  relationTypesArray = {
    0: 'Child',
    1: 'Parent',
    2: 'Spouse',
  ...
  ngOnInit(): void {
    this.findRelationForm = this.fb.group({
      personOne: ['default', [Validators.required, customValidator(/default/)]],
      personTwo: ['default', [Validators.required, customValidator(/default/)]],
    });
    this._langService.currentMessage.subscribe((message) => {
  ...
  rasiResolver(thing: any): Object {
    if (thing == -1) return this.rasiArray[28];
    else return thing;
  }
  ...
  onSubmit() {
    this._loadingService.start();
    this.noPathFlag = false;
    this.infoGetterService
      .getRelationBetween({
        per1: this.findRelationForm.value.personOne,
        per2: this.findRelationForm.value.personTwo,
      })
      .subscribe(
        (res: { opSuccess: Boolean; opContent: Object }) => {
          if (!res) {
            this.treepath = [];
            this.noPathFlag = true;
          } else this.treepath = res.opContent;
